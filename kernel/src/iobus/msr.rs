...
pub fn read_msr(register: u64) -> u64 {
    ...
    unsafe { core::arch::asm!("rdmsr", out("eax") lo, out("edx") hi, in("ecx")register, options(nomem)); }
...
pub fn write_msr(register: u64, value: u64) {
    ...
    unsafe { core::arch::asm!("wrmsr", in("ecx") register, in("eax") lo, in ("edx") hi, options(nomem)) };
}
